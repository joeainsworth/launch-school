module TicTacToe
 ...
 class Board
   ...
   def assign_markers!(square_no, marker)
 ...
 class Player
   ...
   def turn(board)
     ...
     board.assign_markers!(answer, marker)
   end
 ...
 class Computer < Player
   ...
   def two_in_row?(line, board, marker)
     board.square.values_at(*line).map(&:mark).count(marker) == 2
   end

   def select_square(line, board, marker)
     return unless two_in_row?(line, board, marker)
     board.square.select { |k, v| line.include?(k) && v.mark == ' ' }.keys.first
   end

   def assess_options(square, board, marker)
     Board::WINNING_LINES.each do |line|
       square = select_square(line, board, marker)
   ...
   def turn(board, human)
     square = nil
     square = assess_options(square, board, marker) unless square
     square = assess_options(square, board, human.marker)
     square = board.empty_squares.sample unless square
     board.assign_markers!(square, marker)
   end
 end
   ...
   private
     ...
     def current_player_turn
       ...
       if current_player == human
         ...
         current_player.turn(board, human)
